...
union
select count(*) from trips_details2
union
select count(*) from trips_details3;
...
select sum(end_ride) total_completed_trips from trips_details4;
...
select sum(searches_for_quotes) total_searches_for_quotes from trips_details4

--total searches which got quotes

select sum(searches_got_quotes) total_got_quotes from trips_details4

--total driver cancelled
select count(*) - sum(driver_not_cancelled) as total_driver_cancelled from trips_details4

--total otp entered
select sum(otp_entered) total_otp_entered from trips_details4

--total end ride

select sum(end_ride) total_end_ride from trips_details4

--cancelled bookings by driver

select count(*) - sum(customer_not_cancelled) cancelled_bookings from trips_details4
...
(select top 1 faremethod,count(faremethod) total_fare_count from trips
group by faremethod
order by count(faremethod) desc)a
on p.id=a.faremethod

-- the highest payment was made through which instrument
select p.method from payment p inner join
(select top 1 faremethod,fare from trips
order by fare desc)a
on p.id=a.faremethod
...
select * from(
select *,dense_rank() over(order by trip desc) as rank from
...
select * from
(select *,dense_rank() over(order by fare desc) as rank from
...
select* from(
select *,dense_rank() over(order by trip_counts desc) rank from
(select count(tripid) as trip_counts,duration from trips
group by duration)a)b
where rank =1

-- which driver , customer pair had more orders
select * from(
select *,dense_rank() over(order by trip_counts desc) as rank from
(select driverid,custid,count(tripid) as trip_counts from trips
...
select (sum(customer_not_cancelled)*100.0 - count(*))/sum(searches) from trips_details4

-- which area got highest trips in which duration
select duration,loc_from,count(tripid) trip_counts from trips
group by duration,loc_from
order by count(tripid) desc

-- which area got the highest fares, cancellations,trips
select * from(
select  *,dense_rank() over (order by total_fare desc) as rank from
(select loc_from,sum(fare) as total_fare from trips
group by loc_from)a)b
where rank =1

select * from
(select*,dense_rank() over(order by cancelled desc) as rank from
...
select * from
(select *,dense_rank() over (order by total_trips desc) as rank from 
(select loc_from,sum(tripid) as total_trips from trips_details4
group by loc_from)a)b
where rank =1 
...
select * from
(select*,dense_rank() over(order by trips_count desc) as rank from
(select duration,count(tripid) as trips_count from trips
group by duration)a)b
where rank =1
